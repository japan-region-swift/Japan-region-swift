import Foundation

enum EventList {
    static let events: [Event] = [
        Event(name: "神山.swift #1", link: URL(string: "https://kamiyama-swift.connpass.com/event/319105/")!),
        Event(name: "Chiba.swift #1", link: URL(string: "https://chibaswift.connpass.com/event/328367/")!),
        Event(name: "Kanagawa.swift #1", link: URL(string: "https://kanagawaswift.connpass.com/event/332163/")!),
        Event(name: "Chiba.swift #2", link: URL(string: "https://chibaswift.connpass.com/event/331925/")!),
        Event(name: "Osaka.swift #1", link: URL(string: "https://osaka-swift.connpass.com/event/336324/")!),
        Event(name: "Minokamo.swift #1", link: URL(string: "https://minokamo-swift.connpass.com/event/336644/")!),
        Event(name: "Nagoya.swift #1", link: URL(string: "https://japan-region-swift.connpass.com/event/343345/")!),
        Event(name: "HAKATA.swift feat. Japan-\\(region).swift #1", link: URL(string: "https://hakata-swift.connpass.com/event/353191/")!),
        Event(name: "TamaRiver.swift #1", link: URL(string: "https://japan-region-swift.connpass.com/event/357612/")!),
        Event(name: "Kanagawa.swift #2", link: URL(string: "https://japan-region-swift.connpass.com/event/365671/")!),
        Event(name: "【開発合宿】Okinawa.swift×Swift・Kotlin愛好会コラボ合宿 in 沖縄", link: URL(string: "https://japan-region-swift.connpass.com/event/370422/")!),
        Event(name: "Hakodate.swift #1", link: URL(string: "https://japan-region-swift.connpass.com/event/371217/")!),
        Event(name: "Nagoya.swift #2", link: URL(string: "https://japan-region-swift.connpass.com/event/376480/")!),
        Event(name: "Sapporo.swift #1", link: URL(string: "https://japan-region-swift.connpass.com/event/393404/")!),
        Event(name: "TamaRiver.swift #2", link: URL(string: "https://japan-region-swift.connpass.com/event/394189/")!),
        Event(name: "Okinawa.swift #2", link: URL(string: "https://japan-region-swift.connpass.com/event/405463/")!),
        Event(name: "Kanagawa.swift #3", link: URL(string: "https://japan-region-swift.connpass.com/event/389036/")!),
        Event(name: "Saitama.swift #1", link: URL(string: "https://japan-region-swift.connpass.com/event/397259/")!),
    ]

    struct Event: Sendable {
        let name: String
        let link: URL
    }
}
