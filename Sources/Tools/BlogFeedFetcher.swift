...
nonisolated enum BlogFeedFetcher: Sendable {
    @concurrent
    static func fetch() async throws -> [BlogFeed] {
        ...
        return try await withThrowingTaskGroup { group in
            for feed in feeds {
                ...
                group.addTask {
                    let (articleURL, title, imageURL) = try await ArticleFetcher.fetch(url: url)
                    let utcDateString = date.formatted(.iso8601).replacing("Z", with: "+09:00")
                    let date = ISO8601DateFormatter().date(from: utcDateString)!
                    return BlogFeed(url: articleURL, title: title, imageURL: imageURL, date: date)
                }
            }
